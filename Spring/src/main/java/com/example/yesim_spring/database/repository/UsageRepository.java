...
public interface UsageRepository extends JpaRepository<UsageEntity, Long> {
    ...
    @Query(value =
            "SELECT " +
                    "MAX(usage_info.usage_id) AS usage_id, " +
                    "SUM(usage_info.usage_num_sum) AS usage_num, " +
                    "MAX(usage_info.item_id) AS item_id, " +
                    "MAX(usage_info.usage_time) AS usage_time, " +
                    "usage_info.user_seq, " +
                    "MAX(usage_info.total_code) AS total_code " +
                    "FROM (" +
                    "   SELECT iu.usage_id, iu.item_id, iu.usage_time, iu.user_seq, iu.total_code, " +
                    "          IFNULL(SUM(iu.usage_num), 0) AS usage_num_sum " +
                    "   FROM item_usage AS iu " +
                    "   INNER JOIN item AS i ON iu.item_id = i.item_id " +
                    "   INNER JOIN user AS u ON iu.user_seq = u.user_seq " +
                    "   WHERE (i.name LIKE CONCAT('%', :searchVal, '%') OR u.user_name LIKE CONCAT('%', :searchVal, '%')) " +
                    "     AND DATE_FORMAT(iu.usage_time, :dateForm) = DATE_FORMAT(NOW(), :dateForm) " +
                    "   GROUP BY iu.item_id, iu.user_seq" +
                    ") AS usage_info " +
                    "GROUP BY usage_info.user_seq", // user_seq로 그룹화
            nativeQuery = true)
